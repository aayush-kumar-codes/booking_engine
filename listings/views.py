...
class RoomsAvailableView(APIView):
    ...
    def get(self, request):
        ...
        rooms_below_max_price = BookingInfo.objects.filter(price__lt=max_price).order_by('price')

        reserved_rooms = Reservation.objects.filter(Q(check_in__lte=check_in, check_out__gte=check_in) | Q(check_in__lte=check_out, check_out__gte=check_out) | Q(check_in__gte=check_in, check_out__lte=check_out)).values_list('booking_info_id', flat=True)
